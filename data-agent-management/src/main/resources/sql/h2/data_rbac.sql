...
-- 普通测试用户
INSERT INTO sys_user (username, password, nickname, status) VALUES ('testuser', '$2a$10$EixZaYVK1fsbw1ZfbX3OXePaWxn96p36Fz4kdFqBpt.uyVHSq7iSe', '测试用户', 1);

-- 管理员角色关联
INSERT INTO sys_user_role (user_id, role_id) SELECT u.id, r.id FROM sys_user u, sys_role r WHERE u.username = 'admin' AND r.role_key = 'SUPER_ADMIN';
INSERT INTO sys_user_role (user_id, role_id) SELECT u.id, r.id FROM sys_user u, sys_role r WHERE u.username = 'testuser' AND r.role_key = 'NORMAL_USER';
...
-- 超级管理员拥有所有菜单权限
INSERT INTO sys_role_menu (role_id, menu_id) SELECT r.id, m.id FROM sys_role r, sys_menu m WHERE r.role_key = 'SUPER_ADMIN';

-- 超级管理员拥有所有权限
INSERT INTO sys_role_permission (role_id, permission_id) SELECT r.id, p.id FROM sys_role r, sys_permission p WHERE r.role_key = 'SUPER_ADMIN';
