...
CREATE TABLE IF NOT EXISTS sys_agent_auth (
    ...
    update_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
    CONSTRAINT uk_agent_user UNIQUE (agent_id, user_id)
);
CREATE INDEX IF NOT EXISTS idx_sys_agent_auth_agent_id ON sys_agent_auth(agent_id);
CREATE INDEX IF NOT EXISTS idx_sys_agent_auth_user_id ON sys_agent_auth(user_id);
CREATE INDEX IF NOT EXISTS idx_sys_agent_auth_permission_level ON sys_agent_auth(permission_level);
...
CREATE INDEX IF NOT EXISTS idx_sys_password_history_user_id ON sys_password_history(user_id);

COMMENT ON TABLE sys_password_history IS '密码历史记录表';

-- 更新 update_time 触发器
CREATE OR REPLACE FUNCTION update_sys_user_update_time() RETURNS TRIGGER AS $$ BEGIN NEW.update_time = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
CREATE OR REPLACE TRIGGER trg_sys_user_update_time BEFORE UPDATE ON sys_user FOR EACH ROW EXECUTE FUNCTION update_sys_user_update_time();

CREATE OR REPLACE FUNCTION update_sys_role_update_time() RETURNS TRIGGER AS $$ BEGIN NEW.update_time = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
CREATE OR REPLACE TRIGGER trg_sys_role_update_time BEFORE UPDATE ON sys_role FOR EACH ROW EXECUTE FUNCTION update_sys_role_update_time();

CREATE OR REPLACE FUNCTION update_sys_menu_update_time() RETURNS TRIGGER AS $$ BEGIN NEW.update_time = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
CREATE OR REPLACE TRIGGER trg_sys_menu_update_time BEFORE UPDATE ON sys_menu FOR EACH ROW EXECUTE FUNCTION update_sys_menu_update_time();

CREATE OR REPLACE FUNCTION update_sys_permission_update_time() RETURNS TRIGGER AS $$ BEGIN NEW.update_time = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
CREATE OR REPLACE TRIGGER trg_sys_permission_update_time BEFORE UPDATE ON sys_permission FOR EACH ROW EXECUTE FUNCTION update_sys_permission_update_time();

CREATE OR REPLACE FUNCTION update_sys_agent_auth_update_time() RETURNS TRIGGER AS $$ BEGIN NEW.update_time = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
CREATE OR REPLACE TRIGGER trg_sys_agent_auth_update_time BEFORE UPDATE ON sys_agent_auth FOR EACH ROW EXECUTE FUNCTION update_sys_agent_auth_update_time();
