...
public class SqlExecuteNode implements NodeAction {
	...
	public Map<String, Object> apply(OverAllState state) throws Exception {
		...
		log.debug("[SqlExecuteNode] Raw SQL length: {}, isEmpty: {}", sqlQuery != null ? sqlQuery.length() : "null",
		...
		log.debug("[SqlExecuteNode] SQL after trim length: {}, isEmpty: {}",
				sqlQuery != null ? sqlQuery.length() : "null", sqlQuery == null ? "null" : sqlQuery.isEmpty());
		...
		log.debug("[SqlExecuteNode] DB config retrieved: url={}, schema={}, dialectType={}", dbConfig.getUrl(),
		...
		Flux<ChatResponse> displayFlux = Flux.create(emitter -> {
			...
			try {
				...
				try {
					...
					log.warn("[SqlExecuteNode] Chart config enrichment failed, using null display style: {}",
							e.getMessage());
					displayStyleBO = null;
				}
				...
				result
					.putAll(Map.of(SQL_EXECUTE_NODE_OUTPUT,
							PlanProcessUtil.addStepResult(StateUtil.getObjectValue(state, SQL_EXECUTE_NODE_OUTPUT,
									Map.class, new HashMap<>()), currentStep, strResultSetJson),
							SQL_REGENERATE_REASON, SqlRetryDto.empty(), SQL_RESULT_LIST_MEMORY, resultSetBO.getData(),
				...
				log.error("[SqlExecuteNode] Error type: {}, Error message: {}", e.getClass().getSimpleName(),
						errorMessage);
				log.debug("[SqlExecuteNode] DB config used: url={}, schema={}, dialectType={}", dbConfig.getUrl(),
						dbConfig.getSchema(), dbConfig.getDialectType());
	...
	private DisplayStyleBO enrichResultSetWithChartConfig(OverAllState state, ResultSetBO resultSetBO) {
		...
		try {
			...
			log.debug("[SqlExecuteNode] Sample data JSON length: {} (limited to {} rows)", sqlResultJson.length(),
					SAMPLE_DATA_NUMBER);
