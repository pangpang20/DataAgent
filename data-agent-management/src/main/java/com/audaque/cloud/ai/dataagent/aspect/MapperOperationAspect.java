...
/**
 * Mapper操作监控切面 统一处理Mapper方法的返回值检查和异常处理
 */
@Slf4j
@Aspect
@Component
public class MapperOperationAspect {
		...
		try {
			...
			if (result instanceof Integer) {
				...
				if (affectedRows == 0 && isModifyOperation(methodName)) {
					log.warn("Mapper operation affected 0 rows: {}.{} with args: {}", className, methodName, args);
				}
			}
			...
			log.error("Mapper operation failed: {}.{} with args: {}", className, methodName, args, e);
	...
	private void logOperationResult(String className, String methodName, int affectedRows, Object[] args) {
		if (affectedRows > 0) {
			if (isInsertOperation(methodName)) {
				log.debug("Insert operation successful: {}.{} affected {} row(s)", className, methodName, affectedRows);
			}
			else if (isUpdateOperation(methodName)) {
				log.debug("Update operation successful: {}.{} affected {} row(s)", className, methodName, affectedRows);
			}
			else if (isDeleteOperation(methodName)) {
				log.debug("Delete operation successful: {}.{} affected {} row(s)", className, methodName, affectedRows);
			}
		}
	}
	...
	private boolean isInsertOperation(String methodName) {
		return methodName.toLowerCase().startsWith("insert") || methodName.toLowerCase().startsWith("add")
	...
	private boolean isUpdateOperation(String methodName) {
		return methodName.toLowerCase().startsWith("update") || methodName.toLowerCase().startsWith("modify")
	...
	private boolean isDeleteOperation(String methodName) {
		return methodName.toLowerCase().startsWith("delete") || methodName.toLowerCase().startsWith("remove");
	}
