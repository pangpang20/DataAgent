 ...
 */
package com.audaque.cloud.ai.dataagent.connector.impls.dameng;

import com.audaque.cloud.ai.dataagent.bo.schema.*;
import com.audaque.cloud.ai.dataagent.connector.SqlExecutor;
import com.audaque.cloud.ai.dataagent.connector.ddl.AbstractJdbcDdl;
...
public class DamengJdbcDdl extends AbstractJdbcDdl {
	...
	public List<TableInfoBO> showTables(Connection connection, String schema, String tablePattern) {
		// Use ALL_TABLES instead of USER_TABLES to support querying tables from different
		// schemas
		String sql;
	...
	public List<TableInfoBO> fetchTables(Connection connection, String schema, List<String> tables) {
		...
		String sql;
		if (StringUtils.isNotBlank(schema)) {
			sql = "SELECT TABLE_NAME FROM ALL_TABLES WHERE OWNER = '" + schema.toUpperCase() + "' AND TABLE_NAME in("
					+ tableListStr + ")";
		}
		else {
			// Fallback to USER_TABLES if no schema specified (backward compatibility)
	...
	public List<ColumnInfoBO> showColumns(Connection connection, String schema, String table) {
		// Use ALL_TAB_COLUMNS instead of USER_TAB_COLUMNS to support querying columns
		// from different schemas
		String sql;
		if (StringUtils.isNotBlank(schema)) {
			sql = "SELECT COLUMN_NAME, DATA_TYPE, DATA_LENGTH, NULLABLE FROM ALL_TAB_COLUMNS WHERE OWNER = '"
					+ schema.toUpperCase() + "' AND TABLE_NAME='" + table + "'";
		}
		else {
			// Fallback to USER_TAB_COLUMNS if no schema specified (backward
			// compatibility)
			sql = "SELECT COLUMN_NAME, DATA_TYPE, DATA_LENGTH, NULLABLE FROM USER_TAB_COLUMNS WHERE TABLE_NAME='"
					+ table + "'";
		}
	...
	public List<ForeignKeyInfoBO> showForeignKeys(Connection connection, String schema, List<String> tables) {
		...
		String tableListStr = String.join(", ", tables.stream().map(x -> "'" + x + "'").collect(Collectors.toList()));
		// Use ALL_CONSTRAINTS and ALL_CONS_COLUMNS instead of USER_* views to support
		// querying from different schemas
		String sql;
		if (StringUtils.isNotBlank(schema)) {
			sql = "SELECT uc.TABLE_NAME, ucc.COLUMN_NAME, uc.CONSTRAINT_NAME, uc.R_OWNER, uc.R_CONSTRAINT_NAME "
					...
					+ "WHERE uc.CONSTRAINT_TYPE='R' AND uc.OWNER = '" + schema.toUpperCase()
					+ "' AND uc.TABLE_NAME IN (" + tableListStr + ")";
		}
		else {
