...
public class Nl2SqlServiceImpl implements Nl2SqlService {
	...
	public Flux<String> generateSql(SqlGenerationDTO sqlGenerationDTO) {
		...
		int retryCount = sqlGenerationDTO.getRetryCount();

		// Calculate dynamic temperature based on retry count
		// Initial generation: 0.1 (more stable)
		// Retries 1-5: 0.3 (increase diversity)
		// Retries 6+: 0.5 (try different approaches)
		...
		if (sql != null && !sql.isEmpty()) {
			// Use professional SQL error repair prompt with dynamic temperature
			log.debug("Using SQL error fixer for existing SQL with temperature: {}", temperature);
	 ...
	 */
	private double calculateTemperature(int retryCount) {
		if (retryCount == 0) {
	...
	public Flux<ChatResponse> fineSelect(SchemaDTO schemaDTO, String query, String evidence,
		...
		log.info("Schema missing advice: {}",
				sqlGenerateSchemaMissingAdvice != null ? sqlGenerateSchemaMissingAdvice : "(none)");
		log.info("DB config: {}", specificDbConfig != null ? specificDbConfig.getUrl() : "default");
		log.info("Available tables count: {}",
				schemaDTO != null && schemaDTO.getTable() != null ? schemaDTO.getTable().size() : 0);
			...
			try {
				...
				if (!content.trim().isEmpty()) {
					...
					try {
						...
						try {
							List<Map<String, Object>> tableObjects = jsonParseUtil.tryConvertToObject(jsonContent,
									new TypeReference<List<Map<String, Object>>>() {
									});
							log.info("Successfully parsed table objects (new format), count: {}",
									tableObjects != null ? tableObjects.size() : 0);
				...
				if (sqlGenerateSchemaMissingAdvice != null) {
					...
					this.fineSelect(schemaDTO, sqlGenerateSchemaMissingAdvice, additionalTables -> {
						log.info("Adding {} additional tables from advice: {}", additionalTables.size(),
								additionalTables);
