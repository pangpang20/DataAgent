...
import com.audaque.cloud.ai.dataagent.util.McpServerToolUtil;
import com.audaque.cloud.ai.dataagent.util.NodeBeanUtil;
import com.audaque.cloud.ai.dataagent.service.aimodelconfig.AiModelRegistry;
import com.audaque.cloud.ai.dataagent.strategy.EnhancedTokenCountBatchingStrategy;
import com.audaque.cloud.ai.dataagent.workflow.dispatcher.*;
...
public class DataAgentConfiguration implements DisposableBean {
	...
	/**
	 * 配置 Spring MVC 异步任务执行器 替代 SimpleAsyncTaskExecutor，提供线程池复用能力
	 */
	...
	public StateGraph nl2sqlGraph(NodeBeanUtil nodeBeanUtil, CodeExecutorProperties codeExecutorProperties)
		...
		stateGraph.addEdge(START, INTENT_RECOGNITION_NODE)
			...
			// The edge from PlanValidatorNode goes to PlanExecutorNode or
			// HumanFeedbackNode
			.addConditionalEdges(PLAN_VALIDATOR_NODE, edge_async(new PlanValidatorDispatcher()), Map.of(
					...
					// If validation passes and human review enabled, go to human_feedback
					// node
	...
	@ConditionalOnProperty(name = PROJECT_PROPERTIES_PREFIX + ".vector-store.type", havingValue = "simple",
			matchIfMissing = true)
	...
	public org.apache.ibatis.session.Configuration mybatisConfiguration() {
		...
		configuration.getTypeHandlerRegistry()
			.register(com.audaque.cloud.ai.dataagent.enums.AgentStatus.class,
