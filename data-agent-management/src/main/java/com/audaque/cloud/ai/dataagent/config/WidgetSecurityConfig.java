...
/**
 * Widget iframe embedding security configuration. Allows specific paths to be embedded in
 * iframes from any origin.
 */
@Slf4j
@Configuration
		...
		registration.setFilter(new Filter() {
			...
			public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
				...
				if (isWidgetPath(path)) {
					...
					// Some browsers still check X-Frame-Options, so don't set it (or set
					// to
					// ALLOWALL)
