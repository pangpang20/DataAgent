 ...
 * 达梦数据库测试容器配置类
 *
 * 注意：由于达梦数据库的官方 Docker 镜像需要特殊授权和配置， 此配置类作为框架预留，实际使用时需要： 1. 确保有合法的达梦数据库 Docker 镜像 2.
 * 根据实际镜像调整配置参数 3. 或者使用本地安装的达梦数据库进行集成测试
 *
 */
@TestConfiguration
public class DamengContainerConfiguration {
	 ...
	 * 创建达梦数据库测试容器
	 *
	 * 注意：此方法当前被注释，因为达梦数据库 Docker 镜像需要特殊配置 如需启用，请： 1. 取消下面的注释 2. 替换为实际可用的达梦数据库镜像名称 3.
	 * 配置正确的初始化脚本路径
	 */
	// @Bean
	// @ServiceConnection
	public GenericContainer<?> damengContainer() {
		...
		return new GenericContainer<>(DockerImageName.parse("dameng/dm8:latest")).withExposedPorts(DAMENG_PORT)
	...
	/**
	 * 用于本地达梦数据库测试的配置 如果本地已安装达梦数据库，可使用此方法获取连接信息
	 */
