...
/**
 * ModelConfigMapper 测试 - MySQL 和达梦数据库兼容性验证 测试 @sqlDialectResolver@now() OGNL 表达式和
 * LIMIT/CONCAT 语法
 *
 */
@MybatisTest
...
class ModelConfigMapperTest {
	...
	void testFindByConditionsWithConcat() {
		...
		List<ModelConfig> result = modelConfigMapper.findByConditions(null, "turbo", null, null, null);
